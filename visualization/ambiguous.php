...
function set_i($val){
	...
	echo "<a href='ambiguous.php?c=".$sec."'><button>Go to Previous Image</button></a>";
	echo "	</div>";
	echo "  <div class='column' style='background-color:#bbb;'>";
	echo "<a href='ambiguous.php?c=".$one."'><button>Go to Next Image</button></a>";
	echo "	</div>";
	echo "	</div>";
...
function hello($x){	
	...
	$dir    = '/home/sanket/kvqa/QAgeneratingFiles/QA_one';
	#$files1 = scandir($dir);
	$files1=array();
	$files2 = scandir($dir, 1);
	$f = fopen('/home/sanket/kvqa/QAgeneratingFiles/1-face_analysis/ambiguous.csv', "r");
	while(!feof($f)){
		$line = fgets($f);
		array_push($files1, $line);
	}
#print_r($files1);
...
#echo "<img src = 'demo.jpeg'></img>";
	...
	*/
	...
	//$img_id = preg_split("/[-]/", $img_id[0]);
	echo "    <h2>Image: ".$files1[$x+1]."</h2>";
	$myfile = fopen('/home/sanket/kvqa/phase1AnnCloseList.csv', "r");
	$cnt=0;
	while($cnt<$files1[$x+1]){
		$line = fgets($myfile);
		$line = preg_split("/[\t]/", $line);
		$url = $line[0];
		$cap = $line[3];
