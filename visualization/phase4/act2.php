...
$st=$_POST["stInd"];
$en=$_POST["enInd"];
$annFile=sprintf("ann2_left/numFaceAnn-%u-%u.txt", $st, $en);
...
for ($i = 0; $i <sizeof($left_img_ids); $i++) {
    ...
    $txt=$A1.$A2.$A3.$A4.$A5.$A6.$A7.$A8.$A9;
    fwrite($file, $txt);
...
else
{
   $st=$en+1;
   $en=$en+5;
   #$url="http://dosa.cds.iisc.ac.in/VQA-disp/dispWikiVQA/phase3/ann3.php?ST=$st&EN=$en";
   $url = "http://127.0.0.1/kvqa/phase4/ann2.php?ST=$st&EN=$en";
   header("Location: $url"); 
   exit();
}
?>
