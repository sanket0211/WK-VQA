...
$st=$_POST["stInd"];
$en=$_POST["enInd"];
$annFile=sprintf("ann5_left/numFaceAnn-%u-%u.txt", $st, $en);
...
for ($i = 0; $i < sizeof($left_img_ids); $i++) {
	...
	$A12=$_POST['Q12' . $left_img_ids[$i]]."\n";

	$txt=$A1.$A2.$A3.$A4.$A5.$A6.$A7.$A8.$A9.$A10.$A11.$A12;
	fwrite($file, $txt);
...
if(fmod($en,10)==0)
  ...
  $st=$en+1;
  $en=$en+5;
  $url="http://127.0.0.1/kvqa/phase4/ann5.php?ST=$st&EN=$en";
  header("Location: $url"); 
  exit();
  }
?>
